...
def get_optimization_graph(
    loss, learning_rate, max_global_norm, global_step, optimisation_function=tf.train.AdamOptimizer
):
    # Optimisation step
...
def calc_binary_cross_entropy(probs, outputs, weights=1):
    return -tf.reduce_mean((outputs * tf.log(probs + 1e-8) + (1 - outputs) * tf.log(1 - probs + 1e-8)) * weights)
...
def last_relevant_time_slice(output, sequence_length):
    ...
    if len(shape) == 3:
        ...
        index = tf.range(0, batch_size) * max_length + tf.subtract(
            sequence_length, 1
        )  # length should be batchsize as well
...
def save_network(tf_session, model_folder, cp_name, optimisation_summary):
    ...
    save_path = saver.save(tf_session, os.path.join(model_folder, "{0}.ckpt".format(cp_name)), write_meta_graph=False)
...
def add_hyperparameter_results(
    optimisation_summary, model_folder, net_name, serialisation_name, validation_loss_col=None
):
    ...
    df[serialisation_name] = pd.Series({"best_epoch": best_idx, "validation_loss": min_loss})
    save_name = os.path.join(model_folder, net_name + ".csv")
    df.to_csv(save_name)


def load_hyperparameter_results(model_folder, net_name):
    save_name = os.path.join(model_folder, net_name + ".csv")
...
def hyperparameter_result_exists(model_folder, net_name, serialisation_name):
    df = load_hyperparameter_results(model_folder, net_name)
