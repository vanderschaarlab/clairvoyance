...
[20180320 Treatment Effects with RNNs] model_rnn
Created on 21/3/2018 4:33 PM
...
_ACTIVATION_MAP = {"sigmoid": tf.nn.sigmoid, "elu": tf.nn.elu, "tanh": tf.nn.tanh, "linear": lambda x: x}
...
class Seq2SeqDecoderCell(tf.contrib.rnn.RNNCell):
    ...
    def call(self, inputs, state):
        ...
        if self.b_training_mode:
            ...
            actual_states, prev_outputs = tf.split(state, [self.lstm_cell.state_size, self._output_size], axis=-1)
...
class RnnModel:
    def __init__(self, params):
        ...
        with tf.variable_scope(variable_scope_name):
            self.rnn_cell = tf.contrib.rnn.BasicLSTMCell(
                ...
                name=variable_scope_name if variable_scope_name != "network" else None,
            )
            ...
            self.output_b = tf.get_variable(
                "Output_b", [self.output_size], dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer()
            )
        ...
        self.global_step = tf.get_variable("global_step_tfrnn", initializer=0, dtype=np.int32, trainable=False)
    ...
    def _apply_memory_adapter(self, encoder_states):
            ...
            if b_single_layer:
                ...
                self.memory_adapter_layer = {
                    ...
                    "b2": tf.get_variable(
                        "Adapter_Layer2_b",
                        [self.hidden_layer_size * 2],
                        # LSTM memory is double concated
                        dtype=tf.float32,
                        initializer=tf.contrib.layers.xavier_initializer(),
                    ),
                }
        ...
        decoder_states = memory_activation_fxn(
            tf.matmul(encoder_states, self.memory_adapter_layer["W1"]) + self.memory_adapter_layer["b1"]
        )
    ...
    def get_prediction_graph(
        ...
        output = self._build_prediction_graph(
            data_chunk, with_dropout=with_dropout, initial_states=initial_states, b_dump_all_states=b_dump_all_states
        )
    ...
    def _build_prediction_graph(self, data_chunk, with_dropout=True, initial_states=None, b_dump_all_states=False):
        ...
        lstm_additional_size = (
            self.output_size if not self.b_use_seq2seq_training_mode and self.b_use_seq2seq_feedback else 0
        )
        ...
        for i in range(num_samples):
            ...
            if b_dump_all_states:
                ...
                all_states, dumped_states = tf.nn.dynamic_rnn(
                    state_dumping_cell,
                    input_minibatch,
                    initial_state=initial_states,  # None for default
                    dtype=tf.float32,
            ...
            if self.softmax_size != 0:
                ...
                core_outputs, softmax_outputs = tf.split(
                    logits, [self.output_size - self.softmax_size, self.softmax_size], axis=2
                )

                output = tf.concat(
                    [self.output_activation(core_outputs), tf.nn.softmax(softmax_outputs, axis=2)], axis=2
                )
    ...
    def get_training_graph(self, use_truncated_bptt=True, b_stub_front=True, b_use_state_initialisation=True):

        training_dataset = self.training_data.shuffle(buffer_size=10000).batch(self.minibatch_size).repeat(self.epochs)
        ...
        if self.b_use_seq2seq_feedback:
            cell = Seq2SeqDecoderCell(
                cell, self.output_w, self.output_b, b_training_mode=self.b_use_seq2seq_training_mode
            )
        ...
        for chunk_size in chunk_sizes:
            ...
            if states is not None and use_truncated_bptt:
                val, states = tf.nn.dynamic_rnn(
                    cell, input_chunk, sequence_length=sequence_lengths, dtype=tf.float32, initial_state=states
                )
            else:
                val, states = tf.nn.dynamic_rnn(cell, input_chunk, sequence_length=sequence_lengths, dtype=tf.float32)
        ...
        if self.performance_metric == "mse":
            loss = tf.reduce_sum(tf.square(predictions - output_minibatch) * active_entries * weights) / tf.reduce_sum(
                active_entries
            )  # cos some zero entries

        elif self.performance_metric == "xentropy":

            loss = tf.reduce_sum(
                (
                    output_minibatch * -tf.log(predictions + 1e-8)
        ...
        if self.softmax_size > 0:
            loss += tf.reduce_sum(
                softmax_output_minibatch * -tf.log(softmax_predictions + 1e-8) * softmax_active * weights
            ) / tf.reduce_sum(softmax_active)

        optimiser = helpers.get_optimization_graph(
            loss, learning_rate=self.learning_rate, max_global_norm=self.max_global_norm, global_step=self.global_step
        )
        # Parcel outputs
        handles = {"loss": loss, "optimiser": optimiser}
